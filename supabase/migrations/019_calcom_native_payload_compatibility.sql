-- Cal.com native payload compatibility for the private booking ledger.
--
-- Migration 016 remains immutable. This replacement adds one transient
-- null-price provenance input to the narrow RPC, preserves its privileges and
-- stored projection, and accepts only the provider's documented omissions:
-- iCalSequence and iCalUID may be absent on rejection events, and a reschedule
-- may omit iCalUID. Missing rejection sequences are ordered by signed provider
-- event time while preserving the last trusted sequence; explicit sequences
-- retain the original causal ordering. Payment data remains outside the ledger.
...
alter table public.calcom_webhook_events
  drop constraint if exists calcom_webhook_events_review_reason_check;
alter table public.calcom_webhook_events
  add constraint calcom_webhook_events_review_reason_check check (
    ...
    or review_reason in (
      ...
      'unverified_null_price_cancellation'
    )
  );

-- Migration 016 installed the original 24-argument RPC. Remove that callable
-- overload before creating the compatibility signature so service_role retains
-- exactly one ingestion boundary after this migration and after a safe rerun.
-- The final argument defaults false so instances running the previous route can
-- continue calling the RPC during a migration-first rolling deployment.
...
declare
  ...
  sequence_is_inferred boolean := false;
  result_outcome text;
begin
  if p_deployment_target is null
    ...
    or (p_cal_ical_sequence is null
      and p_trigger_event <> 'BOOKING_REJECTED')
    or (p_cal_ical_sequence is not null and p_cal_ical_sequence < 0)
    ...
    or p_provider_price_was_null is null
    or (
      p_provider_price_was_null
      and p_trigger_event <> 'BOOKING_CANCELLED'
    ) then
  ...
  effective_ical_sequence := coalesce(p_cal_ical_sequence, 0);
  ...
  -- Cal's native cancellation payload can replace a verified numeric zero with
  -- null. The null is not proof that the booking was free, so it may only act on
  -- an identity whose earlier signed, accepted non-cancellation receipt proved
  -- the zero-price parser boundary. A first-seen null cancellation is retained
  -- as review evidence and must not synthesize a booking record.
  if manual_review_reason is null and p_provider_price_was_null then
    if resolved_record_id is null or not exists (
      select 1
      from public.calcom_webhook_events as prior_events
      where prior_events.deployment_target = p_deployment_target
        and prior_events.calcom_username = p_calcom_username
        and prior_events.booking_record_id = resolved_record_id
        and prior_events.trigger_event <> 'BOOKING_CANCELLED'
        and prior_events.processing_outcome <> 'manual_review'
    ) then
      manual_review_reason := 'unverified_null_price_cancellation';
    end if;
  end if;
  ...
  if manual_review_reason is null and resolved_record_id is not null then
    ...
    ical_uid_backfill_wins := current_booking.cal_ical_uid is null
      and p_cal_ical_uid is not null;

    if sequence_is_inferred then
      -- Cal omits iCalSequence from some native rejection payloads.
      -- Preserve the last trusted sequence and use the provider timestamp as
      -- the primary clock; an older omission can never outrank newer state.
      effective_ical_sequence := greatest(
        current_booking.details_ical_sequence,
        current_booking.booking_state_ical_sequence
      );
      incoming_details_wins :=
        (p_event_created_at, details_rank)
        > (
          current_booking.details_event_at,
          current_booking.details_event_rank
        );
      incoming_booking_wins :=
        (p_event_created_at, booking_rank)
        > (
    ...
    if current_booking.service_slug <> p_service_slug
      ...
      or (
        sequence_is_inferred
        and (p_event_created_at, details_rank) = (
          current_booking.details_event_at,
          current_booking.details_event_rank
        )
      )
      ...
      or (
        sequence_is_inferred
        and (p_event_created_at, booking_rank) = (
          current_booking.booking_state_event_at,
          current_booking.booking_state_rank
        )
      )
  ...
  if resolved_record_id is null then
    ...
    if record_mutated then
      ...
      set
        ...
        cal_ical_uid = case
          when incoming_details_wins
            then coalesce(p_cal_ical_uid, records.cal_ical_uid)
          when ical_uid_backfill_wins then p_cal_ical_uid
          else records.cal_ical_uid
        end,
        cal_ical_sequence = case when incoming_details_wins
          then effective_ical_sequence else records.cal_ical_sequence end,
        ...
        details_ical_sequence = case when incoming_details_wins
          then effective_ical_sequence else records.details_ical_sequence end,
        ...
        booking_state_ical_sequence = case when incoming_booking_wins
          then effective_ical_sequence
          else records.booking_state_ical_sequence end,
...
revoke all on function public.ingest_calcom_booking_event(
  text, text, text, text, text, timestamptz, text, text, bigint, bigint,
  text, integer, text, text, integer, text, text, text, timestamptz,
  timestamptz, boolean, text, text, text, boolean
)
